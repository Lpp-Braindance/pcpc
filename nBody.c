...
void prepere_recv_buffer(Body *p, Body *p_recv, int own_portion, int start_own_portion, float Fx[], float Fy[], float Fz[])
{
    ...
    // ! end init bodyforce for fist call
}

void bodyForceSplit(Body *p, Body *p2, float dt, int own_portion, int start_own_portion, int start, int end, float Fx[], float Fy[], float Fz[])
{
    for (int i = 0; i < own_portion; i++)
    {
...
void wait_some_work(MPI_Request bcast_recv[], int request_rank_indices[], int requests_ranks[], int worker_to_wait, int rank, Body *p, Body *p_recv, float Fx[], float Fy[], float Fz[], int proc_portion_size[], int proc_portion_start[])
{
    int ready_req = 0;
    ...
    while (ready_req != MPI_UNDEFINED)
    {
        ...
        for (int i = 0; i < ready_req; i++)
        {
            ...
            bodyForceSplit(p, p_recv, dt, own_portion, start_own_portion, start, end, Fx, Fy, Fz);
        }
        MPI_Waitsome(worker_to_wait, bcast_recv, &ready_req, request_rank_indices, MPI_STATUS_IGNORE);
    }
    if (count > 0)
...
int main(int argc, char *argv[])
{
    ...
    int own_portion, start_own_portion;
    int start, end;
    ...
    MPI_Request bsend_corr = MPI_REQUEST_NULL;
    MPI_Request *p_bsend = &bsend_prec;
    ...
    for (int iter = 1; iter <= 10; iter++)
    {
        ...
        MPI_Iallgatherv(p, proc_portion_size[i], MPI_FLOAT, p, recvcounts, displs, MPI_FLOAT, MPI_COMM_WORLD, p_bsend);

        for (i = worker_rank + 1; i < n_workers; i++, j++)
            MPI_Iallgatherv(p, proc_portion_size[i], MPI_FLOAT, p, recvcounts, displs, MPI_FLOAT, MPI_COMM_WORLD, &bcast_recv[j]);

        MPI_Wait(&bsend_prec, MPI_STATUS_IGNORE);

        prepere_recv_buffer(p, p_recv, own_portion, start_own_portion, Fx, Fy, Fz);
        bodyForceSplit(p, p_recv, dt, own_portion, start_own_portion, start, end, Fx, Fy, Fz);
        wait_some_work(bcast_recv, reqs_ranks_indices, reqs_ranks, n_workers - 1, worker_rank, p, p_recv, Fx, Fy, Fz, proc_portion_size, proc_portion_start);
    ...
    if (comm_word_rank == MASTER)
    { //! stampiamo qui fuori perchè quando usciamo dal ciclo for la igather sta ancora lavorando e quindi qui dobbiamo stampare i risultati
        // MPI_Wait(&bsend_prec, MPI_STATUS_IGNORE);
        // MPI_Wait(&igat, MPI_STATUS_IGNORE);
        print_results(comm_word_rank, p, nBodies);
