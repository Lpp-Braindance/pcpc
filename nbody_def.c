...
// // void calculate_gatherv_portions(int recvcounts[], int displs[], int n_workers, int proc_portion_size[], int proc_portion_start[])
// // {
...
// // void print_results(Body *p, int nBodies)
// // {
...
// //         if (argc > 2 && strcmp(argv[2],"-t") == 0 )
// //         {
// //             output_file = fopen("parallel_program", "w+");
// //             print_res = 1;
// //         }
...
// //         // DEFINE BUFFER FOR SEND AND NEW DATA WHILE PREC SEND REQUEST NOT FINISHED
// //         float *buf_send = (float *)malloc(bytes);
// //         Body *p_send = (Body *)buf_send;
// //         // DEFINE TMP VARIABLES
// //         Body *tmp_p_send = p_send;
// //         Body *tmp_buf_swap;
// //         int i, j = 0;
...
// //             wait_some_work(allgath_recv, reqs_ranks_indices, reqs_ranks, n_workers - 1, worker_rank, p, Fx, Fy, Fz, proc_portion_size, proc_portion_start);
// //             // WAITS FOR THE PREVIOUS REQUEST TO WRITE IN THE SEND BUFFER
// //             MPI_Wait(allgath_pointer_prec_req, MPI_STATUS_IGNORE);
// //             prepere_send_buffer(p, p_send, own_portion, start_own_portion);
...
// //             print_results(p_send, nBodies); // stampiamo qui fuori perchè quando usciamo dal ciclo for la igather sta ancora lavorando e quindi qui dobbiamo stampare i risultati
...
//             if(send_req_curr == 0 && send_req_prec == MPI_UNDEFINED)
//             {
...
int main(int argc, char *argv[])
{
    ...
    { 
        ...
        for (int iter = 1; iter <= 10; iter++)
        {
            ...
            // PORTION EXCHANGE BETWEEN PROCESSES: SEND OWN PORTION TO OTHER PROCESSES
            for (i = 0, j = 0; i < worker_rank; i++, j++)
                MPI_Ibcast(&p[proc_portion_start[i]], proc_portion_size[i] * 6, MPI_FLOAT, i, MPI_COMM_WORLD, &allgath_recv[j]);
            // PORTION EXCHANGE BETWEEN PROCESSES: SEND OWN PORTION TO OTHER PROCESSES
            MPI_Ibcast(&p[proc_portion_start[i]], proc_portion_size[i] * 6, MPI_FLOAT, i, MPI_COMM_WORLD, allgath_pointer_next_req);
            // PORTION EXCHANGE BETWEEN PROCESSES: RECEIVE PORTION FROM OTHER PROCESSES
            for (i = worker_rank + 1; i < n_workers; i++, j++)
                MPI_Ibcast(&p[proc_portion_start[i]], proc_portion_size[i] * 6, MPI_FLOAT, i, MPI_COMM_WORLD, &allgath_recv[j]);
            // INIT BODYFORCE FOR ITERATION i
